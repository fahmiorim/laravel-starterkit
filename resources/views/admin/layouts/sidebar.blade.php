...
<aside
    ...
    <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto relative z-10">
        ...
        <a href="{{ route('admin.donor-cards.index') }}"
            class="flex items-center justify-between px-3 py-2.5 mt-2 text-sm font-medium rounded-lg transition-all duration-300 transform hover:scale-105 {{ request()->routeIs('admin.donor-cards.*') ? 'bg-white/20 backdrop-blur-sm shadow-lg' : 'text-white/90 hover:bg-white/10' }}">
            <div class="flex items-center">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
